 ...
 *
 * Run: pnpm run build && pnpm run e2e:claude-code
 *
 * Prerequisites:
 *   - ~/.local/bin/claude available (Claude Code CLI)
 *   - claude auth status authenticated, or ANTHROPIC_API_KEY set
...
async function collectChat(
...
): Promise<{ events: StreamEvent[]; fullText: string; durationMs?: number; costUsd?: number; numTurns?: number }> {
  ...
  console.log();
  return { events, fullText, durationMs, costUsd, numTurns };
}
...
if (!existsSync(claudeBin)) {
  console.log(`${YELLOW}⏭  Claude Code CLI not found at ${claudeBin}, skipping e2e tests.${RESET}`);
  process.exit(0);
}

let authOk = false;
try {
  const authOut = execSync(`${claudeBin} auth status`, { encoding: 'utf-8', timeout: 10000, stdio: ['pipe', 'pipe', 'pipe'] });
  try {
    const authJson = JSON.parse(authOut);
    authOk = authJson.loggedIn === true;
  } catch {
    authOk = authOut.includes('logged in') || authOut.includes('Logged in') || authOut.includes('authenticated');
  }
} catch { /* auth check failed */ }

if (!authOk && !process.env.ANTHROPIC_API_KEY) {
  console.log(`${YELLOW}⏭  Claude Code not authenticated and no ANTHROPIC_API_KEY set, skipping.${RESET}`);
  process.exit(0);
}
...
info(`Auth: ${authOk ? 'logged in (local)' : 'using ANTHROPIC_API_KEY'}`);
...
try {
  const baseDir = await mkdtemp(join(tmpdir(), 'golem-e2e-cc-'));
  dirsToClean.push(baseDir);
  info(`base dir: ${baseDir}`);

  function makeAssistant(dir: string) {
    const opts: { dir: string; engine: string; apiKey?: string } = { dir, engine: 'claude-code' };
    if (!authOk && process.env.ANTHROPIC_API_KEY) opts.apiKey = process.env.ANTHROPIC_API_KEY;
    return createAssistant(opts);
  }
  ...
  console.log(`\n${MAGENTA}${BOLD}  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}`);
  console.log(`${MAGENTA}${BOLD}  PART 1: Core engine — chat, file, Skill, script, memory, metadata${RESET}`);
  ...
  step('Init — one-click init Claude Code assistant');

  try {
    await coreBot.init({ engine: 'claude-code', name: 'golem-claude' });
  ...
  // ── 1.2 Basic chat + multi-turn + Claude Code metadata ──────

  step('Chat + multi-turn + Claude Code metadata (costUsd / numTurns)');

  try {
    const { fullText: t1, durationMs: d1, costUsd: c1, numTurns: n1 } = await collectChat(
      coreBot, 'Introduce yourself in one sentence, under 20 words',
    );
    record('Basic chat success', t1.trim().length > 0);
    record('done has durationMs', d1 != null && d1 >= 0);
    record('done has costUsd (Claude Code specific)', c1 != null);
    info(`durationMs: ${d1}, costUsd: ${c1}, numTurns: ${n1}`);
  ...
  step('Skill injection — .claude/skills/ symlink + CLAUDE.md');
  ...
  try {
    ...
    const claudeSkillsDir = join(coreDir, '.claude', 'skills');
    const hasClaudeSkills = await fileExists(claudeSkillsDir);
    record('.claude/skills/ dir created', hasClaudeSkills);

    if (hasClaudeSkills) {
      const entries = await readdir(claudeSkillsDir);
      record('.claude/skills/ has general', entries.includes('general'));
      record('.claude/skills/ has data-analyst', entries.includes('data-analyst'));
    }

    record('CLAUDE.md generated', await fileExists(join(coreDir, 'CLAUDE.md')));
  ...
  const imDir = await mkdtemp(join(tmpdir(), 'golem-cc-im-'));
  ...
  try {
    await imBot.init({ engine: 'claude-code', name: 'im-support-bot' });
  ...
  const ciDir = await mkdtemp(join(tmpdir(), 'golem-cc-ci-'));
  ...
  try {
    await reviewBot.init({ engine: 'claude-code', name: 'ci-review-bot' });
  ...
  const httpDir = await mkdtemp(join(tmpdir(), 'golem-cc-http-'));
  ...
  try {
    ...
    await httpBot.init({ engine: 'claude-code', name: 'http-bot' });
    info('Initialized http-bot');

    const token = 'e2e-cc-token-' + Date.now();
