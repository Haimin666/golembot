/**
 * GolemBot E2E Test — Codex engine full real-world scenario verification
 *
 * Verifies Codex engine in real user scenarios:
 *   PART 1: Core engine (init, chat, file I/O, Skill via AGENTS.md, session resume)
 *   PART 2: Embedded scenarios (IM Bot, CI/CD code review)
 *   PART 3: HTTP serve (health / auth / multi-user session / reset)
 *
 * Run: pnpm run build && pnpm run e2e:codex
 *
 * Prerequisites:
 *   - codex available (npm install -g @openai/codex)
 *   - OPENAI_API_KEY set (or codex login --with-api-key done)
...
let codexBin = '';
try {
  codexBin = execSync('which codex', { encoding: 'utf-8', timeout: 5000 }).trim();
} catch { /* not found */ }

if (!codexBin) {
  console.log(`${YELLOW}⏭  Codex CLI not found (npm install -g @openai/codex), skipping e2e tests.${RESET}`);
  process.exit(0);
}
...
console.log(`╔══════════════════════════════════════════════════════════════╗`);
console.log(`║  GolemBot E2E — Codex engine full real scenario verification          ║`);
console.log(`╚══════════════════════════════════════════════════════════════╝${RESET}`);
...
try {
  const baseDir = await mkdtemp(join(tmpdir(), 'golem-e2e-cx-'));
  ...
  console.log(`\n${MAGENTA}${BOLD}  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}`);
  console.log(`${MAGENTA}${BOLD}  PART 1: Core engine — init, chat, file I/O, AGENTS.md, session resume${RESET}`);
  ...
  step('Init — one-click init Codex assistant');

  try {
    await coreBot.init({ engine: 'codex', name: 'golem-codex' });
    ...
    // Codex engine should add .codex/ to .gitignore
    const gitignoreContent = existsSync(join(coreDir, '.gitignore'))
      ? await readFile(join(coreDir, '.gitignore'), 'utf-8')
      : '';
    record('.gitignore has .codex/', gitignoreContent.includes('.codex/'));
  } catch (e) {
    record('Init failed', false);
    console.error(e);
  }

  // ── 1.2 Basic chat + multi-turn + session ID ─────────────

  step('Chat + multi-turn + session ID (thread_id)');

  try {
    const { fullText: t1, sessionId: sid1 } = await collectChat(
      coreBot, 'Introduce yourself in one sentence, under 20 words',
    );
    record('Basic chat success', t1.trim().length > 0);
    record('done has sessionId (thread_id)', !!sid1);
    if (sid1) {
      info(`thread_id: ${sid1}`);
    }
  ...
  // ── 1.4 Skill injection via AGENTS.md ──────────────────

  step('Skill injection — AGENTS.md auto-discovery');
  ...
  try {
    ...
    // Regenerate AGENTS.md to include new skill
    await coreBot.init({ engine: 'codex', name: 'golem-codex' }).catch(() => {
      // init throws if golem.yaml exists; re-trigger AGENTS.md refresh via chat
    });

    const { fullText } = await collectChat(coreBot, 'Analyze the data in sales.csv');

    // Codex uses AGENTS.md (not symlinked skill dirs)
    record('AGENTS.md exists at workspace root', await fileExists(join(coreDir, 'AGENTS.md')));
    record('AGENTS.md mentions data-analyst', (await readFile(join(coreDir, 'AGENTS.md'), 'utf-8')).includes('data-analyst'));
  ...
  const imDir = await mkdtemp(join(tmpdir(), 'golem-cx-im-'));
  ...
  try {
    await imBot.init({ engine: 'codex', name: 'im-support-bot' });
  ...
  const ciDir = await mkdtemp(join(tmpdir(), 'golem-cx-ci-'));
  ...
  try {
    await reviewBot.init({ engine: 'codex', name: 'ci-review-bot' });
  ...
  const httpDir = await mkdtemp(join(tmpdir(), 'golem-cx-http-'));
  ...
  try {
    ...
    await httpBot.init({ engine: 'codex', name: 'http-bot' });
    info('Initialized http-bot');

    const token = 'e2e-cx-token-' + Date.now();
  ...
  console.log(`\n${DIM}  Core verified:`);
  console.log(`    Codex engine × GolemBot framework = full Agent experience`);
  console.log(`    File I/O + Skill (AGENTS.md) + memory + IM Bot + CI/CD + HTTP service${RESET}\n`);
