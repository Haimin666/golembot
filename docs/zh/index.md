...
hero:
  ...
  tagline: 一条命令，将 Cursor、Claude Code 或 OpenCode 接入 IM 平台、HTTP API，或嵌入到你的产品中。
  image:
...
features:
  - icon:
      src: /icons/cpu.svg
    title: Agent 就是大脑
    details: GolemBot 不重新发明 AI。它直接使用你已有的 Coding Agent — Cursor、Claude Code 或 OpenCode — 作为引擎。Agent 变强，助手自动变强。
  - icon:
      src: /icons/plug.svg
    title: 随处接入
